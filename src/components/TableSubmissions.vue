...
const onModalDelete = async() => {
    ...
    const url = ref('/services/farmer/' + applicationIdForDelete.value);
    const auth = ref(true);
...
onMounted(() => {
    ...
    if (props.userRole == 'ROLE_CITIZEN') {
        ...
        url.value = '/services/profile/' + farmerId.value + '/my-submissions';
    }else if(props.userRole == 'ROLE_MANAGER') {
        url.value = '/services/manager/submissions';
    }
    performRequest();
});
</script>
