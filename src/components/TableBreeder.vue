...
<template>
    <b-modal v-model="openModalState" id="modal" no-close-on-backdrop="true" no-close-on-esc="true" title-class="display-1" centered hide-header hide-footer @ok="onModalDelete()">        
    ...
    <div class="table-title-container">       
        <h2 class="table-title">Αιτήσεις κτηνοτροφίας</h2>
        ...
        <div class="table-sort-container">
            <b-dropdown size="lg" dropend="" :text="'Ταξινόμηση: ' + sortByOptionGreek" variant="none" class="m-2" toggle-class="btn-dropdown-order" noCaret="">
                <b-dropdown-header>Ημερομηνία</b-dropdown-header>
    ...
    <div class="table-responsive">
        <table v-if="data" class="table align-middle text-start">
            ...
            <tbody v-if="data.length">                                    
                <tr v-for="breeder in onSortBy">
                    <td>
                        <template v-if="props.userRole == 'ROLE_MANAGER' ">
                            <RouterLink :to="{ name: 'manager-breeder', params: { breederId: breeder.id } }" class="me-2">
                                <img :src="viewLogo" />
                            </RouterLink>
                        </template>
                        <template v-else>
                            <RouterLink :to="{ name: 'breeder-info', params: { id: farmerId, breederId: breeder.id } }">
                                <img :src="viewLogo"/>
                            </RouterLink>
                        </template>                        
                    </td>
                    <td>{{ breeder.id }}</td>
                    <td>{{ breeder.creationDate }}</td>
                    <td v-if="breeder.state == 'ACCEPTED'">Εγκεκριμμένη</td>
                    <td v-if="breeder.state == 'REJECTED'">Απορριμένη</td>
                    <td v-if="breeder.state == 'CREATED'">Υπο-επεξεργασία</td>
                    <td>
                        <div class="table-btns-wrap">
                            <template v-if="props.userRole == 'ROLE_MANAGER'">
                                <button v-if="breeder.state == 'CREATED'" class="table-edit-btn">
                                    <RouterLink :to="{ name: 'manager-breeder-update', params: { breederId: breeder.id } }" class="table-edit-btn-link">
                            ...
                            <template v-else>
                                <button v-if="breeder.state == 'CREATED'" class="table-edit-btn">
                                    <RouterLink :to="{ name: 'breeder-update', params: { id: farmerId, breederId: breeder.id } }" class="table-edit-btn-link">
                                        <span class="table-edit-btn-text">Επεξεργασία</span>
                                    </RouterLink>                                         
                                </button>                    
                                <button v-else class="table-btn-offline edit">
                            ...
                            <button v-if="breeder.state === 'CREATED' && (props.userRole === 'ROLE_CITIZEN' || currentRole.includes('ROLE_ADMIN') )" type="button" class="btn-delete" @click="openModal(breeder.id)">Διαγραφή</button> 
                    ...
                    <td class="mr-0">
                        <button v-if="breeder.state == 'ACCEPTED'" type="button" class="btn" @click="onDownload( breeder.id )">
                            <img :src="logo"/>
                        </button> 
                    </td>
                </tr>                        
