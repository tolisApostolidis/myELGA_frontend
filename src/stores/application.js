...
export const useApplicationStore = defineStore('application', () => {
    ...
    const setVerifiedEmail = (tempEmail) => {
        email.value = tempEmail;
    };
    const persistVerifiedEmail = () => {
    ...
    const loadVerifiedEmail = () => {
        let tempEmail = localStorage.getItem('email');
        tempEmail = JSON.parse(tempEmail);
        if (tempEmail === null || tempEmail === undefined) {
            return;
        }
        email.value = tempEmail;
    };
    return { userData, setUserData, persistUserData, loadUserData, clearUserData, isAuthenticated, 
        email, setVerifiedEmail, persistVerifiedEmail, clearVerifiedEmail, loadVerifiedEmail };
});
